"""Config & Options flow for DKN Cloud for HASS (0.4.0a6 hotfix).

Hotfix:
- Reauth: if the context does not provide an entry_id, fallback to the
  single existing entry of this domain so the form still appears.
- Keep options MERGE to preserve hidden keys (notably 'user_token').
"""
...
class AirzoneConfigFlow(config_entries.ConfigFlow, domain=DOMAIN):
                    ...
                    if ok and api.token:
                        return self.async_create_entry(
                            ...
                            data={
                                ...
                                # Temporary compatibility fields (moved to options by setup/migration)
    ...
    async def async_step_reauth(self, entry_data: dict[str, Any]) -> FlowResult:
        ...
        # Fallback: if no entry_id in context, pick the only entry of the domain.
