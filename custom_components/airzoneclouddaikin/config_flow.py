...
class AirzoneConfigFlow(config_entries.ConfigFlow, domain=DOMAIN):
    ...
    async def async_step_user(self, user_input: dict[str, Any] | None = None) -> FlowResult:
        ...
        try:
            ...
            return self.async_show_form(step_id="user", data_schema=_user_schema(user_input), errors={"base": "timeout"})
        except Exception as exc:  # noqa: BLE001
            _LOGGER.warning("Login failed (network/other): %s", type(exc).__name__)
            return self.async_show_form(step_id="user", data_schema=_user_schema(user_input), errors={"base": "cannot_connect"})
        finally:
        ...
        if not isinstance(token, str) or not token.strip():
            return self.async_show_form(step_id="user", data_schema=_user_schema(user_input), errors={"base": "invalid_auth"})
    ...
    async def async_step_reauth_confirm(self, user_input: dict[str, Any] | None = None) -> FlowResult:
        ...
        if user_input is None:
            return self.async_show_form(step_id="reauth_confirm", data_schema=schema, errors={})
        ...
        session = async_get_clientsession(self.hass)
        api = AirzoneAPI(username, session, password=str(user_input[CONF_PASSWORD]), token=None)

        try:
            login_ret = await asyncio.wait_for(api.login(), timeout=60.0)
        except TimeoutError:
            return self.async_show_form(step_id="reauth_confirm", data_schema=schema, errors={"base": "timeout"})
        except Exception:
...
class AirzoneOptionsFlow(config_entries.OptionsFlow):
    ...
    async def async_step_init(self, user_input: dict[str, Any] | None = None) -> FlowResult:
        ...
        next_opts[CONF_SCAN_INTERVAL] = int(user_input.get(CONF_SCAN_INTERVAL, defaults[CONF_SCAN_INTERVAL]))
        next_opts[CONF_EXPOSE_PII] = bool(user_input.get(CONF_EXPOSE_PII, defaults[CONF_EXPOSE_PII]))
