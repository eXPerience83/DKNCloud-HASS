...
What this fixes
---------------
- ERROR "Invalid handler specified": use a single ConfigFlow with `domain=DOMAIN`
  and a top-level OptionsFlow (no nested classes / bad registrations).
- First-setup stores token ONLY in `entry.options['user_token']`; password is never persisted.
- Options MERGE with existing options to preserve hidden keys (like the token).
    ...
    async def async_step_user(
        ...
        return self.async_create_entry(
            ...
            options={
                ...
                CONF_EXPOSE_PII: pii,
                CONF_STALE_AFTER_MINUTES: stale_after,
            },
        )
